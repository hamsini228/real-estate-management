...
const SellerDashboard: React.FC<SellerDashboardProps> = ({ user }) => {
    ...
    const handleAddProperty = async () => {
        ...
        try {
            ...
            // Append property details as JSON string (backend expects "property" part)
            formData.append('property', JSON.stringify({
                ...newProperty,
                images: undefined // We handle images separately via file
            }));
            ...
            if (newProperty.id) {
                // UPDATE Application
                await axios.put(`http://localhost:8081/api/properties/${newProperty.id}`, formData, config);
                setNotification({ msg: 'Property updated successfully! It is pending re-approval.', type: 'info' });
            } else {
                // CREATE Application
                await axios.post('http://localhost:8081/api/properties', formData, config);
                                    ...
                                    <div className="bg-white rounded-4 shadow-sm overflow-hidden h-100 border">
                                        <div className="position-relative" style={{ height: '200px' }}>
                                            <img
                                                ...
                                                onError={(e: any) => {
                                                    console.log('Image failed to load:', p.images);
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="d-flex gap-2">
                                                <Button variant="outline-primary" size="sm" className="flex-grow-1 rounded-pill" onClick={() => {
                                                    setNewProperty(p);
                                                    setImageFile(null); // Clear file on edit init
                                                    setShowAddModal(true);
                                                }}>
                                                    Edit
                                                </Button>
            ...
            <Modal show={showAddModal} onHide={() => setShowAddModal(false)} size="lg" centered>
                ...
                <Modal.Body className="pt-4">
                    <Form>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Property Image (Upload from PC)</Form.Label>
                            <Form.Control
                                type="file"
                                accept="image/*"
                                onChange={(e: any) => setImageFile(e.target.files ? e.target.files[0] : null)}
                            />
                            {newProperty.images && !imageFile && (
                                <div className="mt-2">
